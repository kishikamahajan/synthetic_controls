...
# This plot shows the difference between the treated and the synthetic groups and we can essentially check whether the gap is significantly different from 0 pre and post treatment. The pre-treatment difference is not exactly zero. The gap varies between -0.25 and almost 0.05. However, these pre-treatment differences shouldn't raise much concern as the magnitude of these differences post the treatment is significantly higher which even cross -1 around 1989 and -1 in 1990. 
...
# The result is consistent with what would've been expected. This is because we would expect the post MSPE to pre MSPE ratio to be the highest in the treated group, i.e., we would expect to the most post-treatment effects in the group that was treated. In other words, this proves the point that the negative effects of terrorism in the Basque Country were in fact extreme enough compared to the donor pools' and that this effect observed in Basque Country was unlikely to have occurred by chance. We would've expected the regions in the donor pool to have smaller ratios i.e., the effects should not differ by much in the post treatment period as compared to the pre treatment period. 
...
# As can be seen Basque Country has been given the first rank and when we look at the p-value on Basque, it is approximately 0.056. This is technically above the 5% significance level. Hence, going purely by this table, the treatment effect of the treated unit is not significant.  However, it should be noted that the total number of regions is only 18 and this affects the minimum possible p-value (1/18 ≈ 0.056). For the treatment effect to be "significant" by the 5% level, we need atleast 20 entries as 1/20 = 0.05. Hence, had there been 2 more regions in the donor pool, we could've probably concluded that the effect is "significant". 
